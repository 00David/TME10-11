...
const applyStockChange = async (stock) => {
  ...
  // Avant : if (changeStock < 0) throw new Error('negative stock')
  if (product.attributes.stock+changeStock < 0) throw new Error('negative stock') 

  const data = {
    stock: product.attributes.stock + changeStock,
  }
